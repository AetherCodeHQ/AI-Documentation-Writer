...
> AI-Documentation-Writer - Open source tool by AetherCodeHQ

`ai` `cli` `developer-tools` `documentation` `golang` `open-source`

## What is it?

**Ai Documentation Writer** is a ai/ml tool designed for developers who need fast, reliable, and offline-capable tools. Built with Go for maximum performance and minimal resource usage.
...
### Install from source
```bash
git clone https://github.com/AetherCodeHQ/AI-Documentation-Writer.git
...
```bash
# Basic usage
./AI-Documentation-Writer --path ./target

# With options
./AI-Documentation-Writer --path ./target --format json --output report.json

# Verbose mode
./AI-Documentation-Writer --path ./target --verbose
```
...
### Basic scan
```bash
./AI-Documentation-Writer --path ./my-project
```

### JSON report
```bash
./AI-Documentation-Writer --path ./my-project --format json --output report.json
```
...
- name: Run Ai Documentation Writer
  run: ./AI-Documentation-Writer --path . --format json --output report.json
```
